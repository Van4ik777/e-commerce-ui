      ...
      <Flex mt="40px" ml="70px" mr="70px">
        ...
        <Box ml="80px" style={{ flex: 1 }}>
          ...
          <Flex mt="200" w={800} ml={200}>
            <Button color="red" mr="md" h={60} w={150} style={{ flex: 1, backgroundColor: 'red', color: 'white', fontSize: '18px', borderRadius: 0 }}>
              Buy Now
            </Button>
            <Button variant="outline" color="dark" h={60}  style={{ flex: 1, fontSize: '18px', borderRadius: 0, borderColor: 'black', color: 'black' }}>
              Add to Cart
            </Button>
          </Flex>
        </Box>
      ...
      <Box mt="40px" ml="150px" mr="70px">
        <Tabs defaultValue="description">
          ...
          <Tabs.Panel value="description" pt="xs">
            <Text style={{ fontSize: '20px' }}>{product.description}
            </Text>
          </Tabs.Panel>
          ...
          {product.productDetails && product.productDetails.length > 0 ? (
            <Text style={{ fontSize: '20px', marginBottom: '10px' }}>
              <strong>Colors:</strong> {product.productDetails[0].colors.join(', ') || 'N/A'} <br />
              <strong>Materials:</strong> {product.productDetails[0].materials.join(', ') || 'N/A'} <br />
              <strong>Dimensions:</strong> {product.productDetails[0].height} x {product.productDetails[0].width} x {product.productDetails[0].depth} cm <br />
              <strong>Swivel Mechanism:</strong> {product.productDetails[0].swivelMechanism || 'N/A'}
            </Text>
          ) : (
        ...
        <Tabs.Panel value="reviews" pt="xs">
          {product.reviews && product.reviews.length > 0 ? (
            <Box>
              {product.reviews.map((review: { id: React.Key | null | undefined; userId: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | null | undefined; text: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | null | undefined; }) => (
                <Box key={review.id} mb="md" style={{ borderBottom: '1px solid #e0e0e0', paddingBottom: '10px' }}>
